...
//   modifies this, coin_table.rows[sender]

//   requires sender in coin_table.rows
//   requires forall k :: k in coin_table.rows ==> coin_table.rows[k].balance >= 0.0
// {
...
function method get_balance(account:string): real
  reads this, coin_table, coin_table.rows[account]
  requires account in coin_table.rows
{
  var balance := coin_table.rows[account].balance;
  balance
}
...
method transfer(sender:string, receiver:string, amount:real) returns (r:Status)
  modifies this, coin_table, coin_table.rows[sender], coin_table.rows[receiver]

  // // sender and receiver are present before and after
  requires sender in coin_table.rows
  requires receiver in coin_table.rows
  ensures forall k :: k in old(coin_table.rows) <==> k in coin_table.rows

  // // a simple implication of the transfer
  requires valid_coin_table_rows(coin_table.rows)
  requires coin_table.read(sender).balance >= amount
  // ensures r == Success ==> coin_table.read(receiver).balance >= amount

  ensures r == Success ==>
    conserves_mass(old(coin_table), coin_table, [sender, receiver])
{
  ...
  // coin_table.account_balances_n();

  // var sender_coin := coin_table.read(sender);
  // sender_coin.balance := sender_coin.balance - amount;
  // coin_table.update(sender, sender_coin);
  ...
  // var receiver_coin := coin_table.read(receiver);
  // receiver_coin.balance := receiver_coin.balance + amount;
  // coin_table.update(receiver, receiver_coin);
...
method debit(account:string, amount:real) returns (r: Status)
  modifies coin_table.rows[account]
  ...
  // ensures r == Success ==>
  //   coin_table.read(account).balance
  //     == old(coin_table).read(account).balance - amount
  ensures r == Success ==>
    coin_table.rows[account].balance
      == old(coin_table).rows[account].balance - amount
{
  :- validate_account(account);
  :- enforce(amount > 0.0, "debit amount must be positive");
  :- enforce_unit(amount);
  :- require_capability(DEBIT(account));
  :- enforce(amount <= coin_table.rows[account].balance, "Insufficient funds");

  var coin := coin_table.read(account);
  coin.balance := coin.balance - amount;
  coin_table.update(account, coin);
...
//   modifies this, coin_table.rows[account]

//   requires account in coin_table.rows
//   ensures forall k :: k in old(coin_table.rows) <==> k in coin_table.rows

//   requires forall k :: k in coin_table.rows ==> coin_table.rows[k].balance >= 0.0
//   ensures forall k :: k in coin_table.rows ==> coin_table.rows[k].balance >= 0.0

//   ensures r == Success ==> coin_table.rows[account].balance >= amount
//   ensures r == Success ==>
//     coin_table.rows[account].balance == old(coin_table.rows[account].balance) + amount
// {
//   :- validate_account(account);
//   :- enforce(amount > 0.0, "credit amount must be positive");
//   :- enforce_unit(amount);
...
//   var coin := coin_table.rows[account];
//   coin.balance := coin.balance + amount;
//   // coin_table.rows := coin_table.rows[account := coin];
